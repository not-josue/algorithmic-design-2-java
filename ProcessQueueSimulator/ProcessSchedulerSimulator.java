/*
 * This is the given driver file. Do NOT alter.
 */
...
public class ProcessSchedulerSimulator
{
	...
	public static final int RAND_ADD_VAL = 3;// A value that adds a new process
												// only when it's a multiple.
	public static final int RAND_CANCEL_VAL = 23;// A value that ends the
													// current process only it's
													// a multiple.
	public static Random rng = new Random(10);// Random number generator. Add a
												// number into its parameter to
												// fix its seed value
	public static void main(String[] args)
	{
		...
		for (int i = 0; i < MAX_SIM_TIME; i++)
		{
			...
			{
				printAction("Current Process "
						+ currProcess.toString());
				currProcess.setCompletionTime(
						currProcess.getCompletionTime()
								- 1.0);
				printAction("Updating Completion Time "
						+ currProcess.toString());
			}
			else
			{
				...
				if (currProcess != null)
				{
					printAction("New current process "
							+ currProcess.toString());
				}
				else
				{
	...
	public static void addRandomProcess(
			ProcessScheduler scheduler, int count)
	{
